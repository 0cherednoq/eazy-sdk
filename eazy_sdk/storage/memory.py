...
from eazy_sdk.storage.storage import Storage, StorageMiddleware
...
def MemoryStorage(
    *,
    middlewares: Sequence[StorageMiddleware] = (),
    ...
    ``record_events=True`` appends an :class:`EventRecordingMiddleware` bound to the
    same event store; ``log_ops=True`` appends a :class:`LoggingMiddleware`.
    """
    events = InMemoryEventStore()
    chain: list[StorageMiddleware] = list(middlewares)
    if record_events:
        chain.append(EventRecordingMiddleware(events))
    if log_ops:
