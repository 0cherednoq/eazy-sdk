...
52.1–52.4 исполнены 2026-09-08, фаза закрыта.
...
### 4.4. 52.4 — `Pages.next_url` (подтверждён владельцем 2026-09-08)

`items=` и `next_url: Callable[[T], str | None]`; полей операции стратегия не называет
(`fields == ()`). Шов: декларация операции пересобирается на одну страницу — `path` заменяется
ссылкой, поля с `RequestLocation.PATH`/`QUERY` удаляются из `input_fields`/`input_schema`, их
значения не передаются; заголовки, cookies и тело остаются. Ссылка резолвится против
`NormalizedResponse.url` страницы, которая её отдала, поэтому все страницы, включая первую,
читаются через `with_response=True`. Остановка: пусто, `next_url` вернул `None`.
...
- 2026-09-08 — 52.3 done: `Pages.cursor` (§4.3); 7 тестов (правила, порядок, первая страница без
  токена, цепочка до пустой страницы, async с `max_pages`, объявление).
- 2026-09-08 — 52.4 done по подтверждению владельца («давай и 52.4 next_url»): `Pages.next_url`,
  `NextUrl` как ответ `next_changes`, `_send_page`/`_at_url`/`_next_page` на связанной операции;
  10 тестов (правила, ссылка как есть с сохранением заголовка, относительная ссылка против
  фактического URL, другой хост, `max_pages`, async, объявление). Отклонение от первого
  наброска: относительная ссылка с первой страницы резолвилась против шаблона пути с
  незаполненными `{...}` — заменено на URL из `NormalizedResponse`, ради чего страницы читаются
  с `with_response=True`. Фаза закрыта.
