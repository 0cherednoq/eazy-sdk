...
Все отмечены 2026-09-07 при закрытии фазы; доказательства — в разделе
`## Phase 50` файла `STATUS.md`.

50.1:

- [x] `Query[int]` и `Annotated[int, markers.Query()]` дают одну аннотацию; typing-фикстура зелёная
      на mypy и basedpyright.
- [x] Операция на dataclass, Pydantic и msgspec с одинаковыми именами на проводе даёт байт-в-байт
      одинаковый `PreparedCall`.
- [x] Декоратор и класс дают байт-в-байт одинаковый `PreparedCall`; `Api.op.Operation` публичен.
- [x] `Unpack[TypedDict]`, `responses=`, `response=`, `Wire.projection` не существуют; D-13 даёт подсказку.
- [x] D-01…D-13 воспроизводятся тестами дословно.
- [x] Генератор OpenAPI выдаёт классы операций; снимки обновлены; сгенерированный пакет проходит
      strict mypy и исполняется.
- [x] Абсенс-тест второго пути зелёный.

50.2:

- [x] Все четыре случая `j_html_service.py` работают без единой декларации семьи.
- [x] Специфичность I8 покрыта пятью тестами, включая настоящую ничью.
- [x] `(Model, factory)` поднимает исключение приложения, не наследующее `ApiError`.
- [x] `ApiError` переживает pickle без тела и заголовков.
- [x] `ClientConfig.errors` применяется по хосту и проигрывает сервису и операции.
- [x] `Serialization.documents` выбирает бэкенд по медиа; `html=` не принимается.
- [x] Указатель подписи, не совпадающий с именем на проводе, — D-19.

50.3:

- [x] `request()` не делает I/O; `send(request)` ≡ прямой вызов по байтам; `evolve` работает на трёх
      библиотеках и называет поля при опечатке.
- [x] `options` доступен у `op()`-операции через `send`, у декорированной — параметром.

50.4:

- [x] RPC- и WS-операции объявляются классами; сюиты фаз 49 и `tests/websocket/` зелёные без
      изменения ожиданий.
- [x] Плагин adaptix дампит через реторт и не конфликтует со штатным адаптером.
- [x] 40 страниц docs-site обновлены, `docs_freshness.py check` зелёный, миграция полная.
- [x] `surface_count.py` ≤ baseline; `len(eazy_sdk.__all__) <= 40`.
...
31. **`Rpc.method` не принимает отдельные `errors=`/`fallback=`**, а использует общие
    `_HttpOptions` (2026-09-07): их типы совпадают, а ключ-`int` в `errors=` для RPC — это код
    протокола. Лоуринг в RPC-кейсы включается флагом `_HttpSpec.envelope_cases` и работает только
    когда `success=` не задан явно: операция, назвавшая свои кейсы (как сюита фазы 49), сохраняет их.
32. **Не-HTTP операция публикует себя сама** (2026-09-07): `op()` вызывает `cls.__publish__()`,
    объявленный протоколом `_PublishesItself`. Прямой импорт `eazy_sdk.websocket` из `api.py`
    нарушил бы границу, которую сторожит `test_http_runtime_does_not_import_websocket_boundary`.
    Следствие: `op()` над WS-классом типизирован как возвращающий дескриптор WS-пакета, а его
    `P`/`T` не выводятся — вызов WS-операции типизирован как `Any`.
33. **D-23 проверяется в `AsyncWsApi.__init_subclass__`** (2026-09-07), а не в `__set_name__`
    HTTP-дескриптора — по той же причине границы; проверка структурная (у HTTP-дескриптора есть
    `spec` и `operation_type`).
34. **`WsSubscribe[T]`: `T` — тип сообщения**, а вызов возвращает `Subscription[T]` (2026-09-07);
    `WsCall[T]` возвращает `T`, `WsSend` — `None`.
35. **Adaptix-адаптер подменяет штатный dataclass-адаптер, а не встаёт рядом** (2026-09-07).
    §4.15 предполагал `supports_type` только по `types`, но adaptix читает любой dataclass, и
    штатный адаптер тоже: реестр поднимал `AmbiguousModelAdapterError`. Адаптер отвечает за все
    dataclass, направляет `types` в реторт и передаёт остальные штатной реализации; ставит его
...
38. **Кэш разобранного ответа ключуется значением, а не `id()`** (2026-09-07, ревью фазы 50):
    `ResponseContext.cached` принимает `Hashable` и держит ключ в словаре, поэтому ключ живёт
    ровно столько, сколько запись. Прежний `id(identity)` получал построенный на месте кортеж
    `(extractor, backend)`: кэш не попадал ни разу, документ перепарсивался на каждый
    кандидат-кейс, а освобождённый адрес мог достаться другому объекту. Следствие для плагинов:
    ключ обязан быть хешируемым — `XmlCodec` объявлен `Hashable`-протоколом.
39. **Клиентские кейсы ошибок кэшируются по `(operation_id, host)`** (2026-09-07), а запись
    хранит и исходное объявление: попадание засчитывается только при совпадении по
    идентичности. Прежний ключ `id(contract)` ничего не удерживал, поэтому пересобранный роутер
    (`bind_session_lifecycle` строит свой на каждую scoped-сессию) мог получить чужой контракт,
    а словарь рос без границ.
40. **`ClientConfig.errors` не нормализует себя в `__post_init__`** (2026-09-07): правило фазы 44
    запрещает конфигу мутировать себя, поэтому `__post_init__` только отвергает два написания
    одного хоста, а приведение к нижнему регистру делает фабрика `_runtime_from_boundary`.
41. **`success=`/`errors=` документируют несколько форм списком, а не любой `Sequence`**
    (2026-09-07): кортеж занят формой `(Model, factory)`, поэтому `Spec | list[Spec]` — это то,
    что рантайм действительно раскрывает, и теперь то же говорит тип.
42. **`Json(unwrap=..., extractor=...)` — ошибка объявления** (2026-09-07): pointer читает
    штатный экстрактор, пользовательский разворачивает конверт сам; молча проигнорированный
    аргумент заменён `ValueError`.
43. **`Omittable` значит одно и то же на обоих протоколах** (2026-09-07). Чтобы это стало правдой,
    понадобились три правки: WS `values_of` отбрасывает `UNSET`, `models.load` разворачивает
    PEP 695 alias (`unroll_alias` переехал из `compile/input.py` в `models/adapters.py` — слой
    ниже, дубля нет), а `_normalize_dump` не сериализует поля со значением `UNSET`, потому что
    операция, пересобранная из своих значений, подставляет сентинел за всё, что не передали.
44. **`EndpointLike` объявляет `operation_type`, `projection`, `inject`** (2026-09-07). Компилятор
    доставал их через `getattr`, хотя фаза 50 добавила их полями декларации: причина была в узком
    структурном протоколе. Цена честного протокола — восемь тестовых контрактов объявили те же три
    члена; выигрыш — читается и проверяется, а не угадывается.
45. **D-23 спрашивает тип, а не набор имён атрибутов** (2026-09-07): проверка живёт на WS-стороне
    и импортирует `_OperationDescriptor` локально, потому что граница односторонняя — запрещён
    только импорт WS из HTTP (`test_http_runtime_does_not_import_websocket_boundary`). `op()` по
    той же причине использует объявленный протокол `_PublishesItself`, а не `getattr("__publish__")`.
46. **Арность проекции — свойство объявления** (2026-09-07): её считает компилятор
    (`CompiledContract.projection_arity`), а не `inspect.signature` на каждой сборке тела и на
    каждом ретрае. При этом замер показал, что сам `compile_endpoint` выполняется на **каждый**
    вызов операции (кэша скомпилированного контракта нет, `executor.py`), — это отдельная работа,
    в ревью фазы 50 она не входила.
