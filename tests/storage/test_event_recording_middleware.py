...
from eazy_sdk.storage.middleware import EventRecordingMiddleware
from eazy_sdk.storage.storage import StorageMiddleware, StorageOp


def test_is_a_storage_middleware() -> None:
    assert isinstance(EventRecordingMiddleware(InMemoryEventStore()), StorageMiddleware)


async def test_maps_canonical_ops_to_events() -> None:
    events = InMemoryEventStore()
    mw = EventRecordingMiddleware(events)
    acc = uuid4()
